...
class Elite extends MY_Controller {
    ...
    public function tnc(){
        if(empty($this->session->userdata('memberelite'))){
			...
			redirect(base_url()."elite/main?tnc=0");
		}
	}
    public function tncProses(){
        ...
        if($_FILES['buktiSubscribeYoutube'] && $_FILES['buktiSubscribeYoutube']['name']!=''){
            ...
            $new_name = $this->datamember['id'].'_buktiSubscribeYoutube_'.rand().'.'.$ekstensi;
            ...
            if (!$this->upload->do_upload('buktiSubscribeYoutube')) {
                ...
                // }else{
                    ...
                    $member['hp'] = $up['hp'];
                    $member['instagram'] = $up['instagram'];
                    $member['tiktok'] = $up['tiktok'];
                    $member['bukti_screnshoot_yt'] = $up['bukti_screnshoot_yt'];
                    $member['created_date'] =date('Y-m-d H:i:s');
                    //cek apakah email terdaftar
                    $data_member = $this->model_global->get_data(array('data' => 'row','table' => 'member a','where' => array( 'a.email' =>$member['email'])));
                    if($data_member){
                        $id_member = $data_member['id_member'];
                        //insert log buat gantiin fungsi update
                        $up['id_member'] = $id_member;
                        $up['create_at'] =date('Y-m-d H:i:s');
                        //simpan data
                        $save = $this->model_global->insertId($up, 'elite_member_master');
                    }else{
                        $id_member = $this->model_global->insertId($member, 'member');
                        $save = $id_member;
                    }
                    $this->session->set_userdata('memberelite', $id_member);
                //}
                if($save){
    ...
    public function main(){
        //echo $this->session->userdata('memberelite'); exit;
		if(empty($this->session->userdata('memberelite'))){
			redirect(base_url()."elite/tnc");
		}else{
			$data['website'] = $this->website;
    ...
    public function download($id){
        if(!empty($this->session->userdata('memberelite'))){
            $data['member'] = $this->model_global->get_data(array('data' => 'row','table' => 'member a','where' => array( 'a.id_member' =>$this->session->userdata('memberelite'))));
		}else{
            $data['member'] = $this->model_global->get_data(array('data' => 'row','table' => 'member a','where' => array( 'a.id_member' =>$this->datamember['id'])));
        }
        $data['website'] = $this->website;
