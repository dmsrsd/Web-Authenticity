...
class Profile extends MY_Controller {
	...
	public function index(){
		...
		if(empty($this->datamember)){
			...
			if ($this->session->userdata('memberduoverse')) {
				$url = $this->session->userdata('memberduoverse');
				// Hapus session
				$this->session->unset_userdata('memberduoverse');
				// Redirect
				redirect($url);
			}
			$data['website'] = $this->website;
	...
	public function updatesoundroom(){
		...
		$this->form_validation->set_rules('instagram', 'Intagram Link', 'required');
		$this->form_validation->set_rules('spotify', 'Link Profile Spotify', 'required');
		$this->form_validation->set_rules('manggung', 'Tempat manggung', 'required');
	...
	public function soundroom(){
		...
		if(empty($this->datamember)){
			...
			// }
			if(empty($data['band_old'])){// sudah ikutian aku redirect ke playlistnya 
				$data['band_old'] = $this->model_global->get_data(array('data' => 'row','table' => 'soundroom_2024', 'where' => array('created_by' => $this->datamember['id']),'order_by' => 'id_soundroom desc'));
			}
			$data['website'] = $this->website;
			$data['kategori'] = $this->kategori;
			// $data['provinsi'] = $this->model_global->get_data(array('select' => '*', 'table' => 'kota','group_by' => 'provinsi', 'order_by' => 'provinsi asc'));
	...
	public function submitvote(){
		...
		if(empty($this->datamember)){
			...
			$cek = $this->db->query("SELECT * FROM vote_member_2025 Where id_member='".$this->datamember['id']."'")->result_array();
			if(count($cek) < 1) {
				// Jalankan proses jika jumlah data kurang dari 2
				$data_arr['id_vote'] = $_POST['pilih'];
				$data_arr['id_member'] = $this->datamember['id'];
				$data_arr['name'] = $this->datamember['fullname'];
				...
				$m =  "You have already selected the contestants in advance";
				$ret['status'] = "false";
