		...
		if(empty($this->datamember)){
			...
			if(isset($_POST['submit'])){
				...
				*/
				...
				$cek2 = $this->model_global->get_data(array('data' => 'row','table' => 'soundroom_2023', 'where' => array('slug' => $_POST['slug'], 'approve !='=>'2')));
                $next = "false";
                $sound = "false";
				if(!empty($cek2)){
					if($cek2["approve"]=="2"){
					...
					$cek3 = $this->model_global->get_data(array('data' => 'row','table' => 'soundroom_2023', 'where' => array('created_by' => $this->datamember['id'])));
					if(!empty($cek3)){
						$pending = $this->model_global->get_data(array('data' => 'row','table' => 'soundroom_2023', 'where' => array('approve'=>'0','created_by' => $this->datamember['id'])));
						$sudah = $this->model_global->get_data(array('data' => 'row','table' => 'soundroom_2023', 'where' => array('approve'=>'1','created_by' => $this->datamember['id'])));
						if(!empty($pending)){
				...
				if($next=="true"){
					...
					$insert_id = $this->model_global->insert($_POST, 'soundroom_2023');
					if($insert_id){
						$ret['status'] = "true";
