...
class Soundroom extends MY_Controller {
	...
	public function tiga(){
		...
		$this->load->view('front/podcast/header',$data);
		//$this->load->view('front/soundroom',$data);
		$this->load->view('front/soundroom3',$data);
	}
 	...
 	public function getGalleryLoader3(){
		$response = array();
		$items = array();
		$row = $this->model_global->get_data(array(
			'select' => 'a.*, b.kota,b.provinsi',
		...
		if(isset($row) && count($row) > 0): foreach($row as $r):
			$thumb = base_url()."uploads/soundroom/".$r['image'];
			$img = base_url()."uploads/soundroom/".$r['image'];
			$html = "<article class='col-sm-3 lazyItem'><div class='lazyContent'>
					<div class='btn-porto' style='cursor:pointer;' title='$r[judul]'  data-porto='$img' data-keyboards='false'   data-backdrops='static' data-toggle='modal' data-target='#portoModal'>
						<img  src='".$thumb."' class='img-responsive' style='width:100%'></div>
					<div class='inlazy'>
						<h3 style='margin-top:1px; text-transform:capitalize'>".$r["judul"]."</h3><div class='clearfix'></div>
					</div>
				</div></article>";
