...
class Soundroom extends MY_Controller {
	...
	public function share($id){
        ...
        if (!in_array($year, ['2024', '2023', '2022', '2019'])) {
            $year = '2023';
        }
	...
	public function index(){
		//buat redirect ke 2024
		if(($_GET['year']==2024)or($_GET['year']==2023)or($_GET['year']==2022)){
			$year = $_GET['year'];
		}else{
			redirect(base_url()."soundroom?year=2024");
		}
	...
	public function getGallery(){
        ...
        if (count($soundroom_more) > 0) {
            foreach($soundroom_more as $key=>$row){
				...
				if ($year == '2024') {
					$url_share = base_url()."soundroom/share/".$row['id_soundroom']."?year=2024&utm_source=sroom24&utm_medium=sroom24visitor&utm_campaign=sr24".$row['judul']."&utm_id=sroom24visitor&utm_term=sroom24visitor";
