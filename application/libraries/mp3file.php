...
// Source: https://gist.github.com/jagneshchawla/3803671
class mp3file
{
    protected $block;
    protected $blockpos;
    ...
    protected function readmp3frame()
    {
        ...
        if ($iscbrmp3)
        {
            ...
            $this->mp3data['Length mm:ss'     ] = mp3file::seconds_to_mmss((int) $this->mp3data['Length']);
    ...
    {
        if (isset($this->block[1]) && isset($this->block[2]) && isset($this->block[3])) {
    ...
    {
        if (isset($this->block[1]) && isset($this->block[2])) {
    ...
    protected function containsvbrxing()
    {
        ...
        if (isset($this->block[37]) && isset($this->block[38]) && isset($this->block[39]) && isset($this->block[40])) {
            return(
    ...
    {
        if ((int) $mp3['Sampling Rate']>0)
        {
            return  ceil((144 * ((int) $mp3['Bitrate'])*1000)/((int) $mp3['Sampling Rate'])) + ((int) $mp3['Padding Bit']);
        }
        return 'unknown';
    }
    //-----------------------------------------------------------------------------
