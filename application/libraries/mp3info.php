...
class Mp3info {
     ...
     * @throws \Exception
     */
    public function getMetadata($filename, $parseTags = false) {
        if (self::$_bitRateTable === null)
            self::$_bitRateTable = require dirname(__FILE__).'/mp3info/bitRateTable.php';
        if (self::$_sampleRateTable === null)
            self::$_sampleRateTable = require dirname(__FILE__).'/mp3info/sampleRateTable.php';
    ...
    public function resetMetadata() {
        ...
        $this->isVbr = false;
        $this->hasCover = false;
        $this->vbrProperties = array();
        $this->coverProperties = array();
        $this->channel = null;
        $this->tags = array();
        $this->tags1 = array();
        $this->tags2 = array();
        $this->id3v2MajorVersion = null;
        $this->id3v2MinorVersion = null;
        $this->id3v2Flags = array();
        $this->id3v2TagsFlags = array();
        $this->_fileName = null;
        $this->_fileSize = null;
        $this->_framesCount = 0;
        $this->_parsingTime = null;
        $this->_cbrFrameSize = null;
        $this->_id3Size = null;
    }
    ...
    private function parseAudio($filename, $fileSize, $mode) {
        ...
        if ($mode & self::META) {
            ...
            if ((int) $this->_cbrFrameSize > 0) {
                $framesCount2 = ceil($audioSize / $this->_cbrFrameSize);
            }
            ...
            // }
            if ($this->isVbr && $framesCount !== null && $framesCount != 0) {
                $avgFrameSize = $audioSize / $framesCount;
            
                // Ensure layer version is valid
                if ($this->layerVersion == self::LAYER_3) {
                    $this->bitRate = $avgFrameSize * $this->sampleRate / (1000 * 12);
                } elseif ($this->layerVersion == self::LAYER_1 || $this->layerVersion == self::LAYER_2) { 
                    $this->bitRate = $avgFrameSize * $this->sampleRate / (1000 * 144);
                } else {
                    // Handle invalid layer version
                    $this->bitRate = 0; // or some default value
                }
            } else {
                // Handle cases where VBR is not set or frame count is zero
                $this->bitRate = 0; // or some default value
            }
...
//            }
    ...
    private function readMpegFrame($fp) {
        ...
        $divider = (int) ($this->sampleRate + ($header_bytes[2] >> 1 & 0b1));

        // go to the end of frame
        if ($this->layerVersion == self::LAYER_1) {
            if ($divider > 0) {
                $this->_cbrFrameSize = floor(((12 * $this->bitRate) / $divider) * 4);
            } else {
                $this->_cbrFrameSize = 0;
            }
        } else {
            if ($divider > 0) {
                $this->_cbrFrameSize = floor((144 * $this->bitRate) / $divider);
            } else {
                $this->_cbrFrameSize = 0;
            }
        }

        fseek($fp, (int) ($pos + $this->_cbrFrameSize));
