  ...
  if (navigator.userAgent.match(/Edge/i) || navigator.userAgent.match(/Trident.*rv[ :]*11\./i)) {
	...
	$(document).on('ready', function() {
		...
		?>
			...
			$('#scrablenotif .notif').html("<div class='alert alert-danger' align='center'>Hari ini Anda sudah bermain <b>Simply Scrable</b></div>")  ;
		<?php }else{?>
			$('#scrablestart').modal({backdrop: 'static', keyboard: false})  ;
		<?php }?>
		$('#frmsrable').hide();
		...
		$('.btn-batal').click(function(){
			document.location.href='<?=base_url();?>rewards';
		});
		var sudah = "false";
		...
		$('.btn-mulai').click(function(){
			...
			$.ajax({
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url();?>rewards/startscrable",
				beforeSend: function () {
				...
				success: function (e) {
					if(e.status=="true"){
						...
						$('#scrablestart').modal('hide');

						<?php
						$currentDate = strtotime($date);
						$futureDate = $currentDate+(60 * $website['time_scrable']);
						$formatDate = date("m d, Y H:i:s", $futureDate);
						$pisah = explode(" ",$formatDate);
						...
						if(enda=="0"){
							...
							var pisahs = starta.split(" ");
							var pisahts = pisahs[0].split("-");

							if(browser=="safari"){
								var jadi = pisaht[0] + "/"+pisaht[1]+"/"+pisaht[2]+"T"+pisah[1];
								var jadinow = pisahts[0] + "/"+pisahts[1]+"/"+pisahts[2]+"T"+pisahs[1];
								var countDownDate = new Date(parseUTC(jadi)).getTime();
								var countDownDateNow = new Date(parseUTC(jadinow)).getTime();
							}else{
								var jadi = pisaht[1] + " "+pisaht[2]+", "+pisaht[0]+" "+pisah[1];
								var jadinow = pisahts[1] + " "+pisahts[2]+", "+pisahts[0]+" "+pisahs[1];
								var countDownDate = new Date(jadi).getTime();
								var countDownDateNow = new Date(jadinow).getTime();
							}

							var minusna = countDownDateNow;
							var x = setInterval(function() {
							  var now = new Date().getTime();
							   var countDownDateNow = minusna;
							  var distance = countDownDate - countDownDateNow;
		...
		function acakbos(){
			...
			$.ajax({
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url();?>rewards/scrableacak",
				beforeSend: function () {
					$('.overlay-all').show();
				},
				success: function (e) {
		...
		$("#frmsrable").submit(function (eve) {
			eve.preventDefault();
			var dataform2 = new FormData();
			dataform2.append('<?php echo $this->security->get_csrf_token_name(); ?>', '<?php echo $this->security->get_csrf_hash(); ?>');
			dataform2.append('coount', coount);
			dataform2.append('a', $(".answer").val());
			$.ajax({
				type: "POST",
				data: dataform2,
				dataType: "json",
				contentType: false,
				processData: false,
				url: "<?php echo base_url();?>rewards/scrableanswer",
				beforeSend: function () {
					$('.overlay-all').show();
				},
				success: function (e) {
