    ...
    <div class='container'>
        ...
        <section class="section-participants">
                ...
                <div class='row'>
                    ...
                    <div class='col-sm-9'>
                        <div class='right-filter'>
                            Urut Berdasarkan : <b>
                            <select id='filter2' style='padding:3px 6px; cursor:pointer;margin-top:5px;'>-->
                                <option value='kota'>Kota</option>
                            </select>
                            <select id='provinsi2' style='padding:3px 6px; cursor:pointer;margin-top:5px;'>
                                <option value=''>--</option>
                                <?php
                            ...
                            <select id='kota2' style='padding:3px 6px; cursor:pointer;margin-top:5px;'>
                                <option value=''>--</option>
                            </select>
                            </b>
                        </div>
            ...
            <div class="row participant-list">
                <?php if (count($soundroom) < 1) { ?>
                    <div class='col-md-12'>
                        <div class='soundroom-data'>
                            <div class='text-center'>
                                <h3>Oops! Musik yang lo cari ngga ada!</h3>
                            </div>
                        </div>
                    </div>
                <?php } else { ?>
                    ...
                    <?php foreach($soundroom as $row){
                        ...
                        echo "<div class='box-play'>
                            ...
                            <div class='soundroom-data text-center'>
                                <h1><a href='javascript:void(0);' onClick=\"jumpPlay('$row[id_soundroom]')\" data-band=\"$slug\" data-slug='$row[slug]'>$row[judul]</a></h1>
                                <h2 >$row[kota]</h2>
                            </div>
                        </div>
                    </div>";
...
$(document).on("click", '.sb-heart', function() {
	...
	function jumpPlay(ke){
		...
		if($('#filter1').val()=='kota'){
            ...
            klik($('#play-'+ke).attr('data-band'));
            currentplay($('#play-'+ke).attr('data-slug'));
		}
        ...
        $.ajax({
            ...
            success: function (e) {
                if(e.status=="true"){
                    ...
                    $('#audio').attr('src', e.sound);
                    $('#playDetail').html('<a href="javascript:void(0);" onClick="currentplaytop(this)" class="icon bp1 pl-1 idpl-'+e.soundid+'" data-now="1" data-audio1="a1-'+e.soundid+'" data-audio="'+e.sound+'" data-band="'+e.slug+'" data-slug="'+e.slug+'" data-progress="prog-'+e.soundid+'" data-progress1="prog1-'+e.soundid+'"><i class="fa fa-play"></i></a>');

                    soundbar(e.slug, e.namaband, e.kota, e.img, e.loved);
                    togglePlay($('#playDetail a')[0],"data-progress",e.slug,e.namaband,e.kota,e.img,e.loved);
                }else{
                    // $('#currentPlay').html('No data..!!');
                    $('#currentPlay').html('Oops! Musik yang lo cari ngga ditemukan!');
                }
