...
<div class="panel panel-default">
	...
	?>		
	<form action="<?=base_url()?>cms/logic/posterchallengeProses" method="post" data-parsley-validate enctype="multipart/form-data">
		<input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" style="display: none">
		<div class="panel-body"> 
			<?php if(isset($data['id_posterchallenge']) && $data['id_posterchallenge'] != ''): ?>
				<input type="hidden" name="created_by" value="<?=$data['created_by']?>" />
				<input type="hidden" name="_id" value="<?=$data['id_posterchallenge']?>" />
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-8"> 
						<?php if(isset($data['id_posterchallenge']) && $data['id_posterchallenge'] != '' && $data['image'] != ''): ?>
						<img src='<?php echo base_url();?>uploads/posterchallenge/<?php echo $data['image'];?>' height='150' class='img'>
						<?php endif?>
						<input name="image" type="file" class="form-control" <?php if(isset($data['id_posterchallenge']) && $data['id_posterchallenge'] != ''){}else{echo "required";} ?> >
					</div>
				</div>
			</div>   
