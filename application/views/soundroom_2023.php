			...
			<table class="table table-striped table-bordered table-hover dataTables" >
				...
				<tbody>
					...
					<tr>
						...
						<td align='center'>
							<a href='javascript:void(0);' class="label label-success" id='st-<?php echo $row['id_soundroom'];?>' onclick="change_status('st-<?php echo $row['id_soundroom'];?>','<?=isset($row['status']) ?  $status[$row['status']] : '-'?>','<?=$row['id_soundroom']?>','<?=base_url()?>cms/logic/delete_ajax/soundroom_2023/<?=$row['id_soundroom']?>')">
								<?php echo isset($row['status']) ?  $status[$row['status']] : '-'?>
							</a>
						</td>
						<td align='center'>
							<a href="<?=$url?>soundroom-new?_id=<?=$row['id_soundroom']?>&_year=2023" class='btn btn-xs btn-success  '><i class="fa fa-edit "></i></a>
							<a class='btn btn-xs btn-danger' onclick="return confirm('Are you sure you want to delete this item?');" href="<?=base_url()?>cms/logic/delete/soundroom_2023/<?=$row['id_soundroom']?>?part=write" class="text-danger"><i class="fa fa-trash-o"></i></a>
						</td>
					</tr>
