...
<div class="panel panel-default">
	...
	?>	
	<form action="<?=base_url()?>cms/logic/userProses" method="post" data-parsley-validate enctype="multipart/form-data">
	<input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" style="display: none">
		<div class="panel-body">  
			<?php if(isset($data['id_user']) && $data['id_user'] != ''): ?>
				<input type="hidden" name="_id" value="<?=$data['id_user']?>" /> 
					...
					<div class="col-sm-3"> 
						<select name="group" class="form-control" required>
							...
							<?php  foreach ($group as $key => $value) { ?>
								<option value="<?=$key?>" <?=(isset($data['group']) && $data['group'] == $key) ? 'selected' : ''?>><?=$value?></option>  
							<?php } ?>
						</select>
					</div>
				</div>
