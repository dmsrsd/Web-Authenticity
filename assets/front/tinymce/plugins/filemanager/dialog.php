...
if(isset($_GET['subfolder']) && !empty($_GET['subfolder'])) {
    ...
    $thumbs_cur = $upload_dir . $subfolder.'/';
    $thumbs_path = $current_path . $subfolder.'/';
    if (!file_exists($thumbs_cur.$subdir)) create_folder(false,$thumbs_cur.$subdir);
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
		...
		<input type="hidden" id="cur_dir" value="<?php echo $cur_path; ?>" />
		...
		foreach ($files as $file) {
			...
			<?php }else{ ?>
				    ...
				    <figcaption>
						...
						class="erase-button"
						<?php if($delete_folder){ ?>onclick="if(confirm('<?php echo lang_Confirm_Folder_del; ?>')){ delete_folder('<?php echo $root. $cur_path .$file; ?>','<?php echo $thumbs_path.$subdir .$file; ?>'); $(this).parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>">
			...
			if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
			    ...
			    if(in_array($file_ext, $ext_img)){
                    ...
                    $thumb_path=$current_path.DIRECTORY_SEPARATOR.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;
			    ...
			    if (in_array($file_ext, $ext_video)) {
				    ...
				    $class_ext = 2;
                    $src_thumb = $base_url.$subdir.$file;
...
?>
				...
				<figure>
					...
					<figcaption>
					    ...
					    <?php if($delete_file){ ?>onclick=" if(confirm('<?php echo lang_Confirm_del; ?>')){ delete_file('<?php echo $root. $cur_path .$file; ?>','<?php echo $thumbs_path.$subdir .$file; ?>'); $(this).parent().parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>"><i class="icon-trash <?php if(!$delete_file) echo 'icon-white'; ?>"></i></a>
					    </form>
					</figcaption>
				</figure>
